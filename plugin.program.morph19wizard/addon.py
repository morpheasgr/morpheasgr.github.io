...
def save_check():
	...
	if setting('saveresolveurl')=='true':
		EXCLUDES.append('script.module.resolveurl')
	if setting('saveseren')=='true':
	  EXCLUDES.append('plugin.video.seren')
	if setting('saveadvanced')=='true':
		EXCLUDES.append('advancedsettings.xml')
	return EXCLUDES

def save_move1():
#	if os.path.exists(os.path.join(addondata, addon_id)):
#		shutil.move(os.path.join(addondata, addon_id), os.path.join(packages, addon_id))
	...
	if os.path.exists(os.path.join(addondata,'script.module.resolveurl')):
		shutil.move(os.path.join(addondata, 'script.module.resolveurl'), os.path.join(packages, 'script.module.resolveurl'))
	if os.path.exists(os.path.join(addondata,'plugin.video.seren')):
		shutil.move(os.path.join(addondata, 'plugin.video.seren'), os.path.join(packages, 'plugin.video.seren'))
...
#		if os.path.exists(os.path.join(addondata, addon_id)):
#			os.remove(os.path.join(addondata, addon_id))
#		shutil.move(os.path.join(packages, addon_id), os.path.join(addondata, addon_id))
	...
	if os.path.exists(os.path.join(packages,'script.module.resolveurl')):
		if os.path.exists(os.path.join(addondata, 'script.module.resolveurl')):
			shutil.rmtree(os.path.join(addondata, 'script.module.resolveurl'))
		shutil.move(os.path.join(packages, 'script.module.resolveurl'), os.path.join(addondata, 'script.module.resolveurl'))
	if os.path.exists(os.path.join(packages,'plugin.video.seren')):
		if os.path.exists(os.path.join(addondata, 'plugin.video.seren')):
			shutil.rmtree(os.path.join(addondata, 'plugin.video.seren'))
		shutil.move(os.path.join(packages, 'plugin.video.seren'), os.path.join(addondata, 'plugin.video.seren'))
...
def freshStart():
	...
	if yesFresh:
		...
		if not os.path.exists(packages):
			os.mkdir(packages)
    #remove add-on folder for what we only want to be addon_data excludes
		if setting('saveresolveurl')=='true':
			if os.path.exists(os.path.join(addons_path, 'script.module.resolveurl')):
				shutil.rmtree(os.path.join(addons_path, 'script.module.resolveurl'))
		if setting('saveseren')=='true':
			if os.path.exists(os.path.join(addons_path, 'plugin.video.seren')):
				shutil.rmtree(os.path.join(addons_path, 'plugin.video.seren'))
...
def buildInstall(NAME, NAME2, VERSION, URL):
	...
	if length:
		...
		while True:
			...
			tempzip.write(buf)
			dp.update(size2, 'Downloading your build...' + str(size2) + '/' + str(length2) + 'MB')
				
	else:
		dp.update(50, 'Downloading your build...')
		tempzip.write(zipresp.read())
	if length:
		dp.update(100, 'Downloading your build...' + str(size2) + '/' + str(length2) + 'MB...Done!')
	else:
