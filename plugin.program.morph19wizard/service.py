...
def save_menu():
	save_items = []
	choices = ["Favourites", "Sources", "Resolve URL", "Seren", "Advanced Settings"]
	dialog = xbmcgui.Dialog()
	save_select = dialog.multiselect(addon_name + " - Select items to keep during a build install.",choices, preselect=[0, 1, 2, 3, 4])
	...
	if 'Resolve URL' in save_items:
		setting_set('saveresolveurl','true')
	else:
		setting_set('saveresolveurl','false')
	if 'Seren' in save_items:
		setting_set('saveseren','true')
	else:
		setting_set('saveseren','false')
